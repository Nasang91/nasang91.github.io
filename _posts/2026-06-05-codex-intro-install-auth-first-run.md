...
> 이 글은 **Codex를 처음 사용하는 개발자**를 대상으로 합니다. 목표는 단순 소개가 아니라, 이 글을 따라 하면 최소한 Codex CLI를 설치하고 첫 작업까지 실행해보는 것입니다.
...
예를 들어 “auth 관련 테스트를 보강해줘”, “save/load 버튼을 추가해줘” 같은 식으로 작업이 구체적일수록 좋습니다.
...
입문자는 Codex가 만든 코드 그 자체보다,
다음 항목을 체크하는 습관이 중요합니다.

- 요구사항을 정확히 반영했는가
- 범위를 넘어서 과하게 수정하지 않았는가
- 테스트 또는 검증 기준을 만족했는가
- 설명 없이 이상한 파일까지 건드리지 않았는가
...
이 단계가 중요합니다.
Codex를 잘 쓰는 개발자는 “생성”보다 “검토”를 더 꼼꼼히 합니다.
